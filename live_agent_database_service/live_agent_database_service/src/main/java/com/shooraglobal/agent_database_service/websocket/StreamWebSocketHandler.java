...
public class StreamWebSocketHandler extends BinaryWebSocketHandler {
    ...
    protected void handleBinaryMessage(WebSocketSession session, BinaryMessage message) {
        ...
        boolean written = rtspRelayService.writeStream(deviceToken, streamData);

        // If the write failed the relay has already been stopped; close the WebSocket so the
        // agent's supervisor loop reconnects and a fresh relay is started.
        if (!written) {
            log.warn("RTSP relay write failed for deviceToken={}; closing WebSocket to trigger agent reconnect",
